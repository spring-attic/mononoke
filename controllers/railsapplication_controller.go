...
// +kubebuilder:rbac:groups=apps.mononoke.local,resources=railsapplications,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=apps.mononoke.local,resources=railsapplications/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=apps,resources=deployments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=core,resources=configmaps,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=core,resources=events,verbs=get;list;watch;create;update;patch;delete

func RailsApplicationReconciler(c controllers.Config, registry cnb.Registry) *controllers.ParentReconciler {
	c.Log = c.Log.WithName("RailsApplication")

	return &controllers.ParentReconciler{
		Type: &mononokev1alpha1.RailsApplication{},
		SubReconcilers: []controllers.SubReconciler{
			RailsApplicationResolveImageMetadata(c, registry),
			RailsApplicationApplyOpinions(c),
			RailsApplicationChildDeploymentReconciler(c),
		},
...
func RailsApplicationResolveImageMetadata(c controllers.Config, registry cnb.Registry) controllers.SubReconciler {
	c.Log = c.Log.WithName("ResolveImageMetadata")
	return &controllers.SyncReconciler{
		Sync: func(ctx context.Context, parent *mononokev1alpha1.RailsApplication) error {
...
func RailsApplicationApplyOpinions(c controllers.Config) controllers.SubReconciler {
	c.Log = c.Log.WithName("ApplyOpinions")

	return &controllers.SyncReconciler{
		Sync: func(ctx context.Context, parent *mononokev1alpha1.RailsApplication) error {
			imageMetadata := controllers.RetrieveValue(ctx, ImageMetadataStashKey).(cnb.BuildMetadata)
			applied, err := opinions.Rails.Apply(ctx, parent.Spec.Template, imageMetadata)
			if err != nil {
				return err
			}
...
func RailsApplicationChildDeploymentReconciler(c controllers.Config) controllers.SubReconciler {
	c.Log = c.Log.WithName("ChildDeployment")

	return &controllers.ChildReconciler{
		ParentType:    &mononokev1alpha1.RailsApplication{},
		ChildType:     &appsv1.Deployment{},
		ChildListType: &appsv1.DeploymentList{},

		DesiredChild: func(ctx context.Context, parent *mononokev1alpha1.RailsApplication) (*appsv1.Deployment, error) {
			labels := controllers.MergeMaps(parent.Labels, map[string]string{
				mononokev1alpha1.RailsApplicationLabelKey: parent.Name,
			})
		...
		ReflectChildStatusOnParent: func(parent *mononokev1alpha1.RailsApplication, child *appsv1.Deployment, err error) {
			if err != nil {
